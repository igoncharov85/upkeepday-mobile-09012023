...
const PreviewModal = ({
  ...
  const onDeleteSlot = () => {
    dispatch(deleteSessionClassesAction({ id: currentSessionId as number, schoolId: currentSchool?.SchoolId }))
    handleHideModal();

  };
  const onEditSlit = (change: TClassesChange) => {
    dispatch(updatedSessionClassesAction({ id: currentSessionId as number, change: change, StartDateTime: newTime as string, schoolId: currentSchool?.SchoolId }))
    handleHideModal();
  }
