...
const getStatus = ({present, absent}: {present: boolean; absent: boolean}) => {
  if (present) {
...
export const StudentCheckInModal: FC<AddSessionModalProps> = memo(
  ({visible, visibleHandler, data, hideOwnModal}) => {
    ...
    const onHandleCheckStudent = ({
      ...
      const index = newCheckinsStudent.findIndex(
        (item: ICheckinUser) => item.StudentId === StudentId,
      );
    ...
    const onHandleCheckAll = () => {
      setAllIn(!allIn);
      const newData = checkins.map(item => {
        return {
          StudentId: item.StudentId,
          CheckInStatus: getStatus({present: !allIn, absent: false}),
        };
      });
    ...
    useEffect(() => {
      const newData = checkins.map(item => {
        return {StudentId: item.StudentId, CheckInStatus: item.CheckInStatus};
      });
      setResultCheckinsStudent(newData as []);
      setCheckinsStudent(newData as []);
    }, [checkins]);
                  ...
                  {checkins.map((item, index) => {
                    return (
                      <CheckItem
                        key={item.StudentId}
                        student={item}
                        status={checkinsStudent[index]}
                        onHandleCheckStudent={onHandleCheckStudent}
                      />
                    );
                  })}
                  <View
...
const CheckItem = ({
  student,
  status,
  onHandleCheckStudent,
}: {
  student: ICheckinUser;
  status: any;
  onHandleCheckStudent: any;
}) => {
  ...
  useEffect(() => {
    onHandleCheckStudent({
      ...
      CheckInStatus: getStatus(checkStatus),
    });
  }, [checkStatus]);
    ...
    <>
      <View style={styles.checkItem}>
        ...
        <View style={{flexDirection: 'row'}}>
          <CheckButton
            onPress={handlePress}
            active={checkStatus.present}
            icon={true}
          />
          <View style={{width: 20}} />
          <CheckButton
            onPress={handlePress}
            active={checkStatus.absent}
            icon={false}
          />
          <View style={{width: 25}} />
        </View>
