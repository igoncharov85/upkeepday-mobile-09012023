...
const CustomTimePicker: React.FC<ITimePicker> = ({ visible, data, onSetTime, block }) => {
  ...
  return visible ? (
    <LinearGradient
      ...
      {data.dayPart && (<ItemPicker block={block.dayPart} items={['AM', 'PM']} activeIndex={dayPart == 'AM' ? 1 : 2} onChange={onDayPartChange} />)}
      <TimeLineRight />
    </LinearGradient>
  ) : null;
};
const ItemPicker = memo(({ items, activeIndex, onChange, block }: { items: any[], activeIndex?: number; onChange: (value: any) => void, block?: boolean }) => {
        ...
        <ScrollView
          ...
          {items.map((item, index) => (
            <TouchableOpacity
              key={index}
              style={styles.item}
              onPress={() => block && handleItemPress(index)}>
              <Text
                style={[
                  styles.itemText,
