...
export const WeekTableItem: FC<IWeekTableItem> = ({
  ...
  const onCreateMoreLesson = (event: any, item: any, type: string) => {
    ...
    if (type == 'before') {
      ...
      const prevLessonGlobal = findNextElement(convertArray(daySchedule), {
        start: item.start,
        end: item.end,
      }).previousElement;
    } else if (prevLesson) {
    ...
    navigate(NavigationEnum.SELECT_DURATION_CLASS_MODAL, {
      ...
      maxDuration:
        type == 'before'
          ? subtractTime(item.start, `${timeIndex}:00`)
          : subtractTime(nextLesson?.start, item.end),
    });
  };
    ...
    <>
      <View>
        <View style={styles.containerItem}>
          {lessons.filter(item => item.start.split(':')[0] == timeIndex)
            .length > 0 ? (
            <View
              style={{
                borderRadius: 4,
              ...
              {lessons
                .filter(item => item.start.split(':')[0] == timeIndex)
                .map((item, index) => {
                  return (
                    <>
                      <TouchableOpacity
                        key={`${item.start} ${dayOfWeek}`}
                        style={[
                          styles.lessonTop,
                          {
                            top:
                        ...
                        onPress={event =>
                          onCreateMoreLesson(event, item, 'before')
                        }
                      />
                      ...
                      <TouchableOpacity
                        ...
                        onPress={event =>
                          onCreateMoreLesson(event, item, 'after')
                        }
                      />
                    </>
                  );
            ...
            <TouchableOpacity style={styles.emptySlot} onPress={onHandleSlot} />
          )}
        </View>
      </View>
    </>
