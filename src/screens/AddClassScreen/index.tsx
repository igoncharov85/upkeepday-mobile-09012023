...
export const AddClassScreen: React.FC<IAddClassScreen> = memo(() => {
  ...
  const {createCurrentClassRequest}: any = useAppSelector(
    state => state.schedule,
  );
  const [typeLocation, setTypeLocation] = useState<TypeLocation>(
    createCurrentClassRequest.Location.LocationType &&
      createCurrentClassRequest.Location.LocationType == 'Office'
      ? 1
      : 0,
  );
  ...
  const [classLocation, setClassLocation] = useState(
    createCurrentClassRequest.Location.AddressLine || '',
  );
  ...
  const handleSubmit = useCallback(
    (values: any) => {
      ...
      navigate(NavigationEnum.SELECT_DATE_SCREEN);
    },
